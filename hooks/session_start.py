...
builder, never a live standup.py run).
...
def main():
    ...
    out = run_standup()
    context("SessionStart", header_for(source) + "\n\n" + out)


def _selftest():
    """Checks the pure header builder and that standup.py exists - never
    runs standup.py itself (slow, and it appends to ledgers)."""
    fails = 0
    h = header_for("clear")
